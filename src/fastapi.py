...
from src.infrastructure.workers.inbox_worker import InboxWorker
...
from src.presentation.api.dependencies import (
    ...
    get_process_inbox_use_case,
    get_process_outbox_use_case,
    get_process_shipping_event_use_case,
...
async def lifespan(app: FastAPI):
    ...
    # Main consumer
    save_inbox_use_case = await get_save_inbox_event_use_case(uow_factory)
    _kafka_consumer = await get_kafka_consumer(save_inbox_use_case)
    ...
    # Outbox worker
    process_outbox_use_case = await get_process_outbox_use_case(uow_factory, _kafka_producer)
    ...
    notification_service = get_notification_service(notification_client)
    shipping_use_case = await get_process_shipping_event_use_case(notification_service)
    process_inbox_use_case = await get_process_inbox_use_case(uow_factory, shipping_use_case)
    _inbox_worker = InboxWorker(
        process_inbox_use_case=process_inbox_use_case,
        interval_seconds=5,
        batch_size=100,
    )
